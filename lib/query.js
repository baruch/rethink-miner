...
function applyFilter(q, field, filter) {
  var match = re_gt.exec(filter);
  if (match) {
    q = q.filter(r.row(field).gt(parseInt(match[1])));
    filter = filter.replace(re_gt, '');
  }

  match = re_lt.exec(filter);
  if (match) {
    q = q.filter(r.row(field).lt(parseInt(match[1])));
    filter = filter.replace(re_lt, '');
  }
