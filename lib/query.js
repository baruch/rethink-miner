...
function filterMatched(q, field, filter, rgx, cb) {
  var match = rgx.exec(filter);

  if (match) {
    q = cb(q, field, match[1]);
...
function applyFilter(q, field, filter) {
  res = filterMatched(q, field, filter, re_gt, function (q, field, val) {
    return q.filter(r.row(field).gt(parseInt(val)));
  });
  q = res[0];
  filter = res[1];

  res = filterMatched(q, field, filter, re_ge, function (q, field, val) {
    return q.filter(r.row(field).ge(parseInt(val)));
  });
  q = res[0];
  filter = res[1];

  res = filterMatched(q, field, filter, re_le, function (q, field, val) {
    return q.filter(r.row(field).le(parseInt(val)));
  });
  q = res[0];
  filter = res[1];

  res = filterMatched(q, field, filter, re_lt, function (q, field, val) {
